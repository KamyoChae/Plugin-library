function writer(container) {
    ...
    // 将this指向自身的这个特性保存起来 以解决定时器指向window的问题
    var _this = this
    _this.state = {
    ...
    // 判断是否传进来参数 该参数为dom节点 即打字机纸张
    // 如果想不传入dom节点就运行，那就报个错 
    // 类似没有纸张不能打字
    if (!container) {
        // 如果参数没有传进来 那就抛出异常
    ...
    _this.runEventLoop = function () {
        ...
        // var time = Math.floor(Math.random() * 10) + 10 // 随机生成100 ~ 200之间的正数
        // if (delay < time) {
        //     // 如果延迟时间小于这个随机数 那就结束这个函数 等到页面刷新次数大于这个值 就开始执行下面的打字效果
        //     return
        // }
